...
export type BookingData = {
  ...
  instansi: string;
  jabatan: string;
  roomId: string;       // ← ID ruangan dari DB (FK)
  ruangan: string;      // ← label display ("TULT — Aula Lt.2"), bukan disimpan ke DB
  tanggal: string;
  jamMulai: string;
  jamSelesai: string;
  kegiatan: string;
...
// ── Success Modal ────────────────────────────────────────
function SuccessModal({ data, onClose }: { data: BookingData; onClose: () => void }) {
  return (
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-8 py-10 flex flex-col items-center text-center">
          ...
          <p className="text-[0.95rem] text-grey-500 mb-8">
            Permohonan peminjaman ruangan kamu sedang diproses oleh admin.
          </p>
          <div className="w-full bg-grey-50 border border-grey-200 rounded-xl overflow-hidden mb-8 text-left">
            {[
              ...
              <div key={r.label} className={`flex items-center px-5 py-3 text-sm ${i % 2 === 0 ? "bg-white" : "bg-grey-50"}`}>
                <span className="w-36 font-semibold text-grey-500 flex-shrink-0">{r.label}</span>
...
function BookingWizardInner() {
    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (user) {
        setData((prev) => ({
          ...
          roomId,
          ruangan,
          // Pre-fill dari user metadata (nama & email dari akun Supabase)
          nama:  prev.nama  || (user.user_metadata?.nama  as string) || "",
          email: prev.email || (user.email ?? ""),
        }));
      } else {
        setData((prev) => ({ ...prev, roomId, ruangan }));
      }
    });
  }, [searchParams]);
  ...
  const next = () => { setSubmitError(null); setCurrent((c) => Math.min(c + 1, 4)); };
  const prev = () => { setSubmitError(null); setCurrent((c) => Math.max(c - 1, 1)); };

  const handleSubmit = async () => {
    if (!data.setuju) {
      setSubmitError("Centang persetujuan Syarat & Ketentuan terlebih dahulu.");
      return;
    }
    if (!data.fileSurat) {
      setSubmitError("Upload surat permohonan terlebih dahulu.");
      return;
    }
    if (!data.roomId) {
      setSubmitError("Pilih ruangan terlebih dahulu di langkah 2.");
      return;
    }
    ...
    try {
      ...
      formData.append("roomId",     data.roomId);   // ← pakai roomId, bukan nama
      ...
      if (!res.ok || !result.success) {
        setSubmitError(result.message ?? "Gagal mengirim pengajuan. Coba lagi.");
        return;
      }
      ...
      console.error(err);
      setSubmitError("Terjadi kesalahan koneksi. Periksa internet dan coba lagi.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="bg-white border border-grey-200 rounded-lg shadow-sm overflow-hidden w-full">
        ...
        <div className="p-4 sm:p-8 md:p-16">
          <div className="w-full overflow-x-hidden">
            ...
            {/* Submit error banner */}
            {submitError && (
              <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-md text-sm text-red-700 font-medium">
                ⚠️ {submitError}
              </div>
            )}
            ...
            <div className="pt-6 sm:pt-8 mt-6 sm:mt-8 border-t border-grey-200">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <button
                  ...
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 sm:px-6 py-3 rounded-md bg-white text-grey-700 font-semibold text-sm border border-grey-300 transition-all duration-200 hover:bg-grey-100 cursor-pointer"
                >
                  ← Sebelumnya
                </button>
                ...
                {current < 4 ? (
                  <button
                    type="button"
                    onClick={next}
                    className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 sm:px-6 py-3 rounded-md bg-brand text-white font-semibold text-sm shadow-sm transition-all duration-200 hover:bg-brand-dark hover:-translate-y-px cursor-pointer"
                  >
                    Selanjutnya →
                  </button>
                ) : (
                  <button
                    ...
                    disabled={isLoading}
                    className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 sm:px-6 py-3 rounded-md bg-brand text-white font-semibold text-sm shadow-sm transition-all duration-200 hover:bg-brand-dark cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
                  >
